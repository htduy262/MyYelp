...
class YelpClient: BDBOAuth1RequestOperationManager {
    ...
    func search(with term: String, completion: @escaping ([Business]?, Error?) -> Void) -> AFHTTPRequestOperation {
        return search(with: term, completion: completion)
    }
    
    func search(with term: String, sort: YelpSortMode?, categories: [String]?, deals: Bool?, offset: Int = 0, distance: Double?, completion: @escaping ([Business]?, Error?) -> Void) -> AFHTTPRequestOperation {
